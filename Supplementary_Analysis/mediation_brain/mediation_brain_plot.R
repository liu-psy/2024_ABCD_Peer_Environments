...
mediation_all_brain <- rbind(PFI_volume_brain, DFI_fc_brain, DFI_sub_brain) %>%
  ...
  select(Mediator, independent, dependent, PathA_beta, PathB_beta, PathAB_beta, 
         PathC_beta, PathCC_beta, proportion)
# Brain as mediator
...
# output results
write_csv(mediation_all_brain, "mediations_brain/mediation_all_brain.csv")
...
t.test(mediation_all_brain_common$PathAB_beta, mediation_all_common$PathAB_beta, paired = TRUE)
t.test(DFI_sub$PathAB_beta, DFI_sub_brain$PathAB_beta)
...
ggplot(mediation_plot, aes(V2, V1, color = peers)) +
  ...
  theme_bw() +
  theme_cleveland() +
  scale_color_manual(name = "Independent\nVariables", values = c("#2166AC", "#7FC97F")) +
  guides(
    shape = guide_legend("Independent\nVariables"),
...
ggsave("mediations_brain/sFigure13.svg", width = 24, height = 12, bg = "transparent")

# cortico-subcoritcal RSFCs ----------------------------------------------------
...
ggsave("mediations_brain/sFigure14.svg", width = 24, height = 12, bg = "transparent")
